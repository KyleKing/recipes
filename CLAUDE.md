...
Environment variables are automatically loaded by mise from the `.env` file (configured in `mise.toml`). The `.env` file is auto-generated by `./setup-spacy.sh` and contains platform-specific flags (CGO_LDFLAGS, library paths) needed to link against the spaCy C++ wrapper.
